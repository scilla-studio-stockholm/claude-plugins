...
<style>
  ...
  .swim-col { background: var(--surface); border: 1px solid var(--rule); border-radius: 6px; padding: .55rem; border-left: 3px solid var(--phase-tint-1); min-width: 0; overflow-wrap: anywhere; }
  ...
  .card-detail { margin-top: .55rem; padding-top: .5rem; border-top: 1px dashed var(--rule); overflow-wrap: anywhere; }
  .card-detail blockquote { font-size: .85rem; margin: 0 0 .35rem; padding: .5rem .65rem; border-left: 2px solid var(--accent); overflow-wrap: anywhere; }
  ...
  details#steps .step-phase { font-size: .85rem; font-weight: 600; color: var(--ink); margin: .75rem 0 .25rem; }
  details#steps .step-phase:first-child { margin-top: .25rem; }
  details#steps ol { list-style: none; padding: 0; margin: 0 0 .5rem; }
  details#steps li { font-family: ui-monospace, "SF Mono", Menlo, monospace; font-size: .8rem; padding: .15rem 0; padding-left: 1.2rem; }
  details#steps li code { color: var(--accent); }
  details#steps .step-num { display: inline-block; width: 1.2rem; margin-left: -1.2rem; color: var(--ink-soft); font-weight: 600; }
...
<details id="steps">
  ...
  <!-- Group steps by phase. One <div class="step-phase"> header per phase,
       then an <ol> with one <li> per step in that phase. Steps are numbered
       sequentially across all phases (1, 2, 3...) for at-a-glance ordering.
       Only list steps referenced by at least one card. -->
  <div class="step-phase"><phase name></div>
  <ol>
    <li id="step-<step_id>"><span class="step-num">1.</span><code>step-X-Y</code> — <step name></li>
  </ol>
  <div class="step-phase"><next phase name></div>
  <ol>
    <li id="step-<step_id>"><span class="step-num">N.</span><code>step-X-Y</code> — <step name></li>
  </ol>
</details>
...
- **Step index (`#steps`).** Grouped by phase: one `<div class="step-phase">` header per phase, then an `<ol>` with one `<li>` per step in that phase. Steps are numbered sequentially across all phases (1, 2, 3...) via a `<span class="step-num">` prefix for at-a-glance ordering. Only steps referenced by at least one card are listed. Order: upstream `phases[].steps[]` traversal order. Wrapped in a `<details>` collapsed by default so it doesn't push the journey content down on first view.
