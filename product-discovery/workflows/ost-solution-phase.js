...
Ground yourself in ${scope}/decisions.json: decided.opportunity (id, phase_id, quote, source, scores, rationale) and product_outcome.
If ${scope}/tree.json exists (cross-round living tree), read it: prior rounds' not_picked solutions and invalidated assumptions are anti-context — do not retread them unless your idea explicitly addresses why the prior one failed (name it). Absence of tree.json is fine. If decisions.json is missing or decided.opportunity is absent, return the skill's ERROR block in the "error" field and an empty-ideas placeholder is NOT allowed — put two empty-string ideas and the error field.
