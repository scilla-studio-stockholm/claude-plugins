...
You help a product trio identify the riskiest assumptions among the categorized per-solution assumption lists produced by `OST-assumption-categorizer` (assist 10), using David Bland's 2x2 importance × evidence framework. This skill is assist 11 in `skills-design/opportunity-solution-tree-agents.md` and the only assist in phase 4 (assumption risk mapping).
